...
impl Imu {
    ...
    pub fn sample(&mut self) -> Result<([f32; 3], [f32; 3]), Error> {
        let acc_raw = self.dev.smbus_read_i2c_block_data(LSM6DSL_OUTX_L_XL, 6)?;
        ...
        let gyr_raw = self.dev.smbus_read_i2c_block_data(LSM6DSL_OUTX_L_G, 6)?;
